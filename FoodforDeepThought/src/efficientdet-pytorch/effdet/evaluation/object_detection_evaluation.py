...
class ObjectDetectionEvaluation:
    ...
    def evaluate(self):
        ...
        accumulated_precision_within_bound = None
        accumulated_recall_within_bound = None
        precision_per_class = None
        recall_pr_curve = np.arange(0.0, 1.1, 0.1)
        ...
        for class_index in range(self.num_class):
            ...
            if len(precision_within_bound) > 0:
                precision_pr_curve = np.interp(recall_pr_curve, recall_within_bound, precision_within_bound)

                if precision_per_class is None:
                    precision_per_class = precision_pr_curve
                else:
                    precision_per_class = np.vstack((precision_per_class, precision_pr_curve))

                if accumulated_precision_within_bound is None and accumulated_recall_within_bound is None:
                    accumulated_precision_within_bound = precision_within_bound
                    accumulated_recall_within_bound = recall_within_bound
                else:
                    accumulated_precision_within_bound = np.hstack(
                        (accumulated_precision_within_bound, precision_within_bound)
                    )
                    accumulated_recall_within_bound = np.hstack((accumulated_recall_within_bound, recall_within_bound))
        ...
        precision_pr_curve = np.mean(precision_per_class, axis=0)

        fig, ax = plt.subplots()
        ax.scatter(
            accumulated_recall_within_bound,
            accumulated_precision_within_bound,
            marker='.', s=1, color='lightsteelblue'
        )
        ax.plot(recall_pr_curve, precision_pr_curve, color='r', lw=2.0)
        ax.set_title('EfficientDet Precision-Recall Curve')
        ax.set_xlabel('Recall')
        ax.set_ylabel('Precision')
        plt.savefig(fname='efficientdet-precision-recall-curve.png', bbox_inches='tight', dpi=300)
        plt.show(block=False)
        plt.pause(30)

        return dict(
            mean_ap=mean_ap,
            mean_corloc=mean_corloc,
            precision_pr_curve=precision_pr_curve,
            recall_pr_curve=recall_pr_curve,
            per_class_ap=self.average_precision_per_class,
            per_class_precision=self.precisions_per_class,
            per_class_recall=self.recalls_per_class,
            per_class_corlocs=self.corloc_per_class
        )
